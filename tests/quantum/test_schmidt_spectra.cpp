...
// Verifies that schmidt_spectrum() and all_contiguous_spectra() return
...
bool ghz_single_site_half_half() {
    ...
    bool ok = true;
    for (int k = 1; k <= N; ++k) {
        auto spec = schmidt_spectrum(psi, k, k);
...
bool bell_phi_plus_center() {
    ...
    auto spec = schmidt_spectrum(psi, 1, 1);
...
bool single_site_equals_left_bond() {
    ...
    // (½, ½) — but the more useful sanity check is that schmidt_spectrum
    ...
    auto our_spec = schmidt_spectrum(psi, 1, 1);
