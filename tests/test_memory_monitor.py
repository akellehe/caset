...
from tessera.utils import memory_monitor
from tessera.utils.memory_monitor import (
    MemoryMonitor,
...
class TestMonitorConstruction(unittest.TestCase):
    ...
    @mock.patch("tessera.utils.memory_monitor._read_proc_self_rss_kb", return_value=None)
    @mock.patch("tessera.utils.memory_monitor._read_meminfo_total_kb", return_value=16_000_000)
    ...
    @mock.patch("tessera.utils.memory_monitor._read_proc_self_rss_kb", return_value=100_000)
    @mock.patch("tessera.utils.memory_monitor._read_meminfo_available_kb", return_value=8_000_000)
    @mock.patch("tessera.utils.memory_monitor._read_meminfo_total_kb", return_value=16_000_000)
    ...
    @mock.patch("tessera.utils.memory_monitor._read_proc_self_rss_kb", return_value=100_000)
    @mock.patch("tessera.utils.memory_monitor._read_meminfo_available_kb", return_value=8_000_000)
    @mock.patch("tessera.utils.memory_monitor._read_meminfo_total_kb", return_value=None)
...
class TestKillConditions(unittest.TestCase):
    ...
    def _run_monitor_once(self, rss_kb, avail_kb, total_kb, **kwargs):
        ...
        with mock.patch("tessera.utils.memory_monitor._read_proc_self_rss_kb", side_effect=lambda: fake.get_rss()), \
             mock.patch("tessera.utils.memory_monitor._read_meminfo_available_kb", side_effect=lambda: fake.get_avail()), \
             mock.patch("tessera.utils.memory_monitor._read_meminfo_total_kb", side_effect=lambda: fake.get_total()), \
             mock.patch("tessera.utils.memory_monitor.os.kill", side_effect=lambda pid, sig: kills.append((pid, sig))):
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_none_rss_skips_check(self):
        ...
        with mock.patch("tessera.utils.memory_monitor._read_proc_self_rss_kb", return_value=100_000), \
             mock.patch("tessera.utils.memory_monitor._read_meminfo_total_kb", return_value=16_000_000):
            mon = MemoryMonitor(check_interval=0.02)

        # Now make RSS unreadable during the loop
        with mock.patch("tessera.utils.memory_monitor._read_proc_self_rss_kb", side_effect=flaky_rss), \
             mock.patch("tessera.utils.memory_monitor._read_meminfo_available_kb", return_value=400_000), \
             mock.patch("tessera.utils.memory_monitor.os.kill") as mock_kill:
            time.sleep(0.1)
            mon.stop()
    ...
    def test_none_avail_skips_check(self):
        ...
        with mock.patch("tessera.utils.memory_monitor._read_proc_self_rss_kb", return_value=13_000_000), \
             mock.patch("tessera.utils.memory_monitor._read_meminfo_available_kb", return_value=None), \
             mock.patch("tessera.utils.memory_monitor.os.kill") as mock_kill:
            time.sleep(0.1)
            mon.stop()
            mock_kill.assert_not_called()
    ...
    def test_total_kb_none_skips_critical_pct(self):
        ...
        with mock.patch("tessera.utils.memory_monitor._read_proc_self_rss_kb", return_value=100_000), \
             mock.patch("tessera.utils.memory_monitor._read_meminfo_available_kb", return_value=400_000), \
             mock.patch("tessera.utils.memory_monitor._read_meminfo_total_kb", return_value=None), \
             mock.patch("tessera.utils.memory_monitor.os.kill", side_effect=lambda p, s: kills.append((p, s))):
            mon = MemoryMonitor(check_interval=0.02)
            time.sleep(0.15)
            mon.stop()
    ...
    def test_stop_on_noop_monitor(self):
        ...
        with mock.patch("tessera.utils.memory_monitor._read_proc_self_rss_kb", return_value=None), \
             mock.patch("tessera.utils.memory_monitor._read_meminfo_total_kb", return_value=None):
            mon = MemoryMonitor()
            mon.stop()
