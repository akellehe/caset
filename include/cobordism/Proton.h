...
/// singlet on at least `minEmergentHoles` (default 3) emergent holes. Carrying the
/// singlet is the physical criterion; the hole count is a topological precondition of
/// the period readout this construction uses, and is not a count of quarks. The accessors
...
class Proton {
 public:
  ...
  ///   * `registerDegree` — the color register degree `k` (3 on a 4-manifold, where the
  ///                        register is `ker L_{d-1}`, a spectral subspace; the periods
  ///                        are read over the emergent holes, which are not the register).
  ...
  ///                        score-guided search for the cells to remove (and the
  ///                        boundary facets to cap), instead of
  ...
  /// carries the singlet on `≥ minEmergentHoles` emergent holes (or `maxRestarts` is
  ...
  /// relaxation in one loop: an init
  ...
  /// `seed + i`) until the whole cobordism carries the singlet on `≥ minEmergentHoles`
  /// emergent holes, or `maxRestarts` is exhausted keeping the best attempt. Populates the same
  ...
  /// colorTolerance`) on `≥ minEmergentHoles` emergent holes. Triggers `build()`.
  ...
  /// The emergent holes (`(k+2)`-vertex tuples) on the proton (the whole), over which
  /// the singlet periods are read — `≥ minEmergentHoles` when converged. A topological
  /// observable of this construction, not a quark count. Triggers `build()`.
