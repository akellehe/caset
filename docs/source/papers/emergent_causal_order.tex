...
%  methodology behind the causal-order comparison experiment in
%  tessera.quantum.  Builds from undergraduate linear algebra and quantum
...
\item \textbf{Weak falsification.} If $\preceq_{\maj}$ disagrees with
      ...
      This test requires a non-regular causet, and is out of scope
      for the experiment of Section~\ref{sec:results}.
...
The experiment reported below tests criterion 1 (strong falsification)
on a regular chain.  Criterion 2 requires a re-build of the
Hamiltonian on a non-regular causet, which is outside the present
scope.
...
detail are pointed to the source repository (\texttt{tessera.quantum})
...
% =====================================================================
\section{The causal-order comparison experiment}\label{sec:results}
...
\bottomrule
\end{tabular}
\caption{Scan: Kendall $\tau$ between
...
without being ``outside the cone'' in any geometric sense.  The count
$n_{\maj \notin \LR}$ reported here aggregates same-time and true
cone-overflow contributions; resolving the two requires a refinement
of the agreement code.
...
a causal order is not designed to see; the natural response is to
...
the same state on a non-regular causet and recompute the table.  That
...
\item \textbf{Single Trotter seed.}  All numbers above are point
      estimates from one DMRG/TDVP trajectory per regime.  Bootstrapping
      over Trotter seeds and over the spatial origin $i_0$ of the
      quench would be needed before quoting these numbers in any
      external context.
...
the hypothesis.  A re-build on a non-regular causet is the test
that does.
...
\texttt{tessera} repository under \texttt{tessera.quantum}.  The two
