...
optional and require a working build of the \texttt{tessera.quantum}
subsystem.
...
\begin{problem}[$\star$]
Let $M = \sum_{n=1}^{N} Z_n$ be the total magnetization (which by
the Jordan--Wigner correspondence equals $N - 2\sum_n c^\dagger_n c_n$,
so a conserved $M$ means a conserved total fermion number).  Show
that $M$ commutes with each of the three pieces of the Hamiltonian:
$H_{\mathrm{hop}}$, $H_m$, and $H_E$.  (For $H_{\mathrm{hop}}$ the
key intermediate identity is $[Z_n + Z_{n+1},\, X_n X_{n+1} + Y_n Y_{n+1}] = 0$;
prove it using $[Z_n, X_n] = 2iY_n$, $[Z_n, Y_n] = -2iX_n$.)
\end{problem}
...
that might survive the result on the regular chain.  For
each refinement:
\begin{enumerate}[label=(\alph*),nosep]
...
\textit{Reference: paper~\S8.  Requires \texttt{tessera.quantum} build.}

\begin{problem}
Run \texttt{python examples/quantum/lightcone\_vs\_majorization.py} from the
\texttt{tessera} repository root.  Verify that your output reproduces
