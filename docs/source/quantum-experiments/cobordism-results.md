...
> `realizable_image_sweep.py` (the gate table) — all under
> `examples/cobordism/`.
...
## The realizable gate set

When is an operation $U$ on $Z(T^2)=\mathbb{C}[H^1(T^2;\mathbb{Z}_2)]=\mathbb{C}^4$
DW-realizable? Exactly when it is one of the **six holonomy permutations** — the
...
permuting the three non-trivial classes $\{[a],[b],[a{+}b]\}=\{1,2,3\}$. Two
twisted cylinders generate the group (`realizable_image_sweep.py`): the coordinate
**swap** $(1\,2)$ on the 9-vertex product torus (its mod-2 reduction is the modular
$S$) and the order-3 **multiplier** $(1\,2\,3)$ on the 7-vertex Möbius torus. Every
permutation of the three non-zero vectors of $(\mathbb{Z}_2)^2$ is automatically
$GF(2)$-linear, so the realizable image is the *full* $S_3$. Realizability is a
property of the holonomy action, indifferent to a gate's name:
...
permutes $\{1,2,3\}$) is the *only* property that separates with no errors —
**19 TP / 0 FP / 0 FN / 210 TN over 229 operations**. Every weaker predicate leaks
(*is a permutation* admits the 18 that move index 0; *fixes the trivial class*
admits diagonal sign matrices; *near-$I_4$* catches only the seven identity
copies). The realizable rate by family makes the same point:
...
## Emergent topology: $b_1$ as an output

The construction above *pins* the bulk topology in advance; it need not. At the
Hodge-qubit degree $k=1$, handing the realizability search a **topology-changing
surgery move** — a boundary-fixed interior-cell *remove* — promotes the bulk's
first Betti number $b_1$ from input to **output**
(`emergent_bulk_realizability.py`): from a $b_1=0$ disk seed, on which the
boundary meridian floors ($r\approx0.45$), the search opens the handle ($b_1:0\to1$)
**on its own** under harmonic-residual minimization, with $\partial W$ held
bit-exact, and the realizable harmonic then carries (the meridian realizes on the
annulus, $b_1=1$, at $r\approx7\times10^{-8}$, $\lambda\to0$; the surgery-grown
handle realizes it at $r\sim10^{-5}$). Removal is the load-bearing move —
boundary-fixed *additive* growth is topology-preserving at $k\ge1$ — so the
realizable set is exactly $\mathrm{image}\big(H^1(W)\to H^1(\partial W)\big)$ for
the $W$ the search grows, complementing the construction with a topology that
selects itself.
...
python examples/cobordism/realizable_image_sweep.py       # the gate table
