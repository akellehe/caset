...
% Long inline-code identifiers (cobordism::DijkgraafWitten, RealizabilityOracle
% .decide(), ...) cannot hyphenate and otherwise spill into the right margin;
% let TeX loosen a line as a last resort instead of running over.
...
\section{The realization map \texorpdfstring{$\geo(\cdot)$}{geo}}\label{sec:geo}

Informally, $\geo$ is the \emph{realization} (synthesis) map that
turns quantum data into geometry --- the inverse of the usual ``read a spectrum
off a given complex'' pipeline. It acts on two kinds of input.

\begin{definition}[The realization map]\label{def:geo}
\emph{On states.} For a normalized quantum state $\psi$, $\geo(\psi)$ is a closed
oriented \emph{weighted} simplicial surface $\Sigma$ that carries $\psi$ as a
harmonic form: $\psi \in \ker L_1\big(\geo(\psi)\big)$. The amplitudes and
relative phase of $\psi$ are encoded as the edge weights and holonomies of that
harmonic $1$-form (\S\ref{sec:qubit}). A single qubit realizes as the torus
($g=1$, $b_1=2$); a $g$-qubit state as a genus-$g$ surface.

\emph{On operations.} For an operation $U:\HH_B\!\to\!\HH_A$, $\geo(U)$ is a
cobordism (a manifold-with-boundary) whose interior weighted triangulation
carries the Choi-bent operator $\vecop(U)$ (\S\ref{sec:choi}) as a boundary
harmonic; this is the bulk $\WAB$ of the Overview.
\end{definition}

\noindent
Four properties pin down how $\geo$ behaves; they are what the later sections
silently assume.

\begin{itemize}
\item[\textbf{(G1)}] \textbf{Boundary-compatibility.} Restriction of an operation
  to its boundary states commutes with $\geo$:
  $\partial\,\geo(U) = \overline{\geo(\psi_B)} \sqcup \geo(\psi_A)$, the outgoing
  geometry of $\psi_A$ together with the orientation-reversed incoming geometry
  of $\psi_B$. (This is hypothesis (H2) of the Overview.) So the state-level and
  operation-level halves of Definition~\ref{def:geo} agree on shared boundaries.
\item[\textbf{(G2)}] \textbf{Metric-laden.} $\geo$ outputs a \emph{weighted}
  complex --- edge weights and $U(1)$ phases, i.e.\ a metric and a connection,
  not just a triangulation. The spectral value $Z_{\mathrm{spec}}$ depends on
  these weights, in contrast to the Dijkgraaf--Witten road
  (\S\ref{sec:dw}), which discards them.
\item[\textbf{(G3)}] \textbf{Defined implicitly, by spectral realization.} There
  is no closed formula for $\geo$. A complex $W$ \emph{is} $\geo(U)$ exactly when
  the eigenvector residual $r(W)$ of \S\ref{sec:residual} vanishes --- when the
  bent target is an eigenvector of the boundary Hodge Laplacian. The inverse
  problem of \S\ref{sec:residual} \emph{is} the constructive content of $\geo$;
  the realizability oracle is the search that evaluates it.
\item[\textbf{(G4)}] \textbf{Partial and non-unique.} $\geo(U)$ need not exist:
  it is defined only on the \emph{realizable} operations. For an obstructed $U$
  the residual floors above zero and $\geo(U)$ is undefined --- the floor is the
  non-existence certificate (\S\ref{sec:residual}). When it does exist it is not
  unique: gauge/metric freedom and boundary-fixed Pachner moves yield many
  witnesses with $r(W)=0$, and $\geo(U)$ denotes any one of them.
\end{itemize}

\noindent
With $\geo$ in hand the three hypotheses of the next section read directly:
(H1) $\geo(U)$ is a cobordism, (H2) its boundary is $\geo$ of the two states
(G1), and (H3) its invariant is the amplitude. In the codebase $\geo$ on
operations is realized by \texttt{EigenstateSynthesis} /
\texttt{RealizabilityOracle.decide()} (\S\ref{sec:residual}) with the bend
supplied by \texttt{ChoiJamiolkowski} (\S\ref{sec:choi}); the witness is returned
as a \texttt{Spacetime} and \texttt{getBoundary()} recovers $\partial\,\geo(U)$.
...
\emph{pinned}: $\partial W$ is the synthesized $\geo(\psi_A)$, $\geo(\psi_B)$ and
the output surface, held fixed. The free parameters $\theta$ are the \emph{interior}
...
\noindent
% Dense run-on of long inline-code identifiers; localize the line-breaking
% slack here rather than loosening the whole document.
\begin{sloppypar}
...
\paragraph{Where this lives.} \texttt{HodgeLaplacian} (\S3--\S4),
\texttt{DijkgraafWitten} (\S5--\S6), \texttt{ChoiJamiolkowski} (\S7), and
\texttt{RealizabilityOracle}/\texttt{EigenstateSynthesis}/\texttt{LevenbergMarquardt}
(\S8) are in place; the preparation map of \S\ref{sec:counting}, the $k{=}1$
