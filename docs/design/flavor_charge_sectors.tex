...
\emph{symmetric continuation of the primal $1$-skeleton}: a triangle whose base sits at
$t=0$ has its apex (tip) at $t=1$, and that tip is shared with an inverted triangle based
at $t=2$ --- the alternating up/down tetrahedral (tetrahedral--octahedral) stacking. It
is label-independent and yields genuine vertical worldline (temporal) edges.
...
one care point is the octahedral cells of the stacking: they must be tetrahedralized
without reintroducing a label-dependent diagonal (use the orientation anchor of
\S\ref{sec:orientation}, or symmetrize over the octahedral diagonals, or document the
residual symmetry honestly).

\paragraph{Faithfulness.} The residual
$\norm{M\Pin-\Pout M}/\norm{M}$ must drop well below the $4.3\times10^{-2}$ prism value
and the singlet overlap rise above the $0.999$ floor, while $b_1=11$, the dual complex
stays a valid manifold, and charge conservation stays exact. \#413 owns ``singlet
$\to1$''.
...
Two questions follow, both testable with machinery that already exists --- the prism
extrusion accepts a cumulative \emph{twist} permutation, i.e.\ an orientation-reversing
tube that is a geometric antisymmetrizer.
\begin{itemize}[nosep]
...
\section{The dimensional question (\#418)}
\label{sec:dim}

The full structure of this program wants more than the current geometry supplies. The
electromagnetic field strength has its full $\vec E,\vec B$ content, and the Dirac algebra
its full $4\times4$ gammas, only in $3+1$ dimensions --- a genuine $S^3$ spatial slice
extruded in time. The present construction is $2+1$ dimensional ($S^2\times I$), a reduced
sector: in $2+1$ D the field strength has three components ($B$ is a scalar) and the
Clifford algebra is $\C l(2,1)$ ($2\times2$ gammas). This is a spike, not a build: it
scopes what the Hodge/Regge/relaxation machinery needs to generalize to a triangulated
$S^3$, the cost, and --- crucially --- which observables genuinely \emph{require} $3+1$ D
versus which are faithful already in $2+1$ D (the Gauss-law charge and the binary flavor
label can be explored first in $2+1$ D; full $\vec E,\vec B$ and $4\times4$
Dirac--K\"ahler cannot). The deliverable is a go/no-go recommendation and a minimal $S^3$
prototype with a smoke test.
...
\midrule
$G_1$ & proton present & singlet overlap $\ge 0.999$ (prism), $\to1$ after \#413\\
