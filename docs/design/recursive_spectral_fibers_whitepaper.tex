...
points of the existing Regge and Hodge functionals; the connection phase
relaxes against the Aharonov--Bohm operator, the operator it acts on. Since
that operator's spectrum is fixed by the gauge conjugation above, a
stationarity condition read from its spectrum is constant along gauge orbits,
so the phase relaxes without drifting along a gauge direction.
