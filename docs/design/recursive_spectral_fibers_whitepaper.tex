...
\begin{itemize}
  ...
  \item a complex squared length and one two-level occupation mode on each edge;
...
register is an isometry to machine precision and that its spectral value
...
$\HK_{e}=\operatorname{span}\{\lvert 0\rangle_{e},\lvert 1\rangle_{e}\}$. The
phase of $z_{e}$ is not a second link field; it is part of the existing complex
edge geometry. Saying that an edge ``carries a qubit'' means that it carries
this local mode algebra, not that the global state is forced to be a product of
normalized vectors $q_{e}$.
...
rotation path is not left abstract: it is the documented total-space spin
holonomy cycle already used for the $J^{2}$ readout
(\code{joint\_proton\_spin\_findings.md}), executed as a closed loop and
normalized against its own co-moving, non-rotating reference. If an emergent
...
\begin{itemize}
  ...
  \item a sharp total-space spin readout:
    ...
    accepted spin lift;
  \item finite radius and stable spectral mass/form-factor readouts; and
...
representation of the present theory, and every particle certificate ---
including $\langle J^{2}\rangle$ and its variance --- is a finite Wick sum over
it.
