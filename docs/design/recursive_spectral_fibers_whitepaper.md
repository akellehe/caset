...
The integration specification *Recursive Spectral Fibers from the Chain-Level Whitney Hodge Pencil* \[43\] fixes the metric, operator, frame, recursion, anchor, and Lorentzian conventions this paper cites as “specification” below, and the scaling verification plan \[44\] records the geometric-fidelity measurements that selected the Whitney metric. Both are vendored beside this paper in `docs/design`; the Python oracle whose values the C++ tests pin stays on the repository’s `issue-attachments` release.
...
The ordinary positive-definite Hodge theorem does not extend by slogan to an arbitrary complex bilinear weight. Isotropic cancellations can enlarge or move the kernel, and even the coordinate representative of a degree-zero harmonic class need not be the constant vector. Accordingly this paper never identifies $\ker L_k^H$ with $H_k$ without a separate nondegeneracy theorem. That theorem is the rank-condition proposition of the integration specification \[43\] (its Prop. 4.2, conditions (R1)–(R4) on the chain-level Whitney pencil): under (R1)–(R2) the chain space splits as $\operatorname{im}\partial_k^\sharp\oplus\operatorname{im}\partial_{k+1}\oplus H_k$ with $\dim H_k=b_k$, and under (R1)–(R4) $\ker L_k^H=H_k$ with no Jordan block at zero. The weight is fixed there as $W_k=M_k^{-1}$, the inverse of the sparse Whitney mass matrix of the squared edge lengths, applied by solves and never formed (specification §4). Betti numbers are computed from incidence; spectral fibers are computed from the declared operator. A fiber may be non-harmonic and may live on a contractible cluster. This separation is what makes the no-hole hypothesis mathematically meaningful rather than terminological.
...
#### Degree-zero fibers on 3-simplices (proposed identification).

Beside the edge-mode carrier, a four-level system is proposed to be carried as a band of the covariant degree-zero operator $h_0(z,U)$ anchored to a 3-simplex (Section <a href="#sec:quarks" data-reference-type="ref" data-reference="sec:quarks">10</a>, the tetrahedral anchor): the state is the band’s geometric image on the tetrahedron’s four vertices, with coefficients that come from the squared lengths and connection values alone. Levels are spatial 3-complexes with the Lorentzian protocol of Section <a href="#sec:state" data-reference-type="ref" data-reference="sec:state">3</a> as the default (a $2+1$ signature at each level); time is not a fourth simplex dimension but the succession of levels (Section <a href="#sec:master" data-reference-type="ref" data-reference="sec:master">15</a>). The harmonic band at trivial holonomy is the vertex-volume chain and carries no state; a single tetrahedron carries only the states orthogonal to it in the pencil pairing, so a state with nonzero constant component requires growth of the carrier. This is a proposed identification, not a derivation.

Because $h_k$ is generically complex and non-normal, a band is selected by a closed contour $\Gamma_C$ in the complex spectral plane, not by sorting real parts or imaginary parts. For a Lorentzian complex the band is selected on the Kontsevich–Segal allowable side of the boundary: the instance is computed as the family $s_e(\varepsilon)$ with the timelike squared lengths rotated by $e^{-2i\varepsilon}$ at one or more reported $\varepsilon>0$, a result at $\varepsilon=0$ is reported only alongside its gap certificate and never alone, and extrapolation to $\varepsilon\to0$ is a separate, labeled step (specification \[43\], §10). Its exact Riesz projector is $$P_C=\frac{1}{2\pi i}\oint_{\Gamma_C}
       (\zeta I-h_C)^{-1}\,d\zeta ,
...
g_C^{-1}\widetilde\Phi_C^{\mathsf T}$, leaving $P_C$ unchanged. The left frame is not an independent datum. With the Whitney chain metric $G_1=M_1^{-1}$ it is the geometric image of the right frame, $\widetilde\Phi_C=G_1\Phi_C$ once $\Phi_C^{\mathsf T}G_1\Phi_C=I$, and for a connection $U$ it is the image $\widetilde\Phi_C=G_1^{U^{-1}}\Phi_C^{\vee}B_C(U)^{-\mathsf T}$ of the same band’s frame $\Phi_C^{\vee}$ for the dual connection $U^{-1}$, defined exactly when the complex bilinear restriction $B_C(U)=(\Phi_C^{\vee})^{\mathsf T}G_1^{U}\Phi_C$ is nonsingular; its vanishing is the isotropic band, the only place where the general biorthogonal machinery is needed (specification \[43\], Prop. 6.1). The cell representation of a gauge transformation is $G_k(g)=\rho_k(g)=\operatorname{diag}(g_{b(\sigma)}^{-1})_{\sigma\in K_k}$ with base vertex $b(\sigma)=\min\sigma$ (specification Def. 5.1).
...
Let $P_{\ell}=\{C_{v}^{\ell}\}$ be an intrinsic partition at scale $\ell$ into persistent connected components. At $\ell=0$ the object is the microscopic simplicial complex $K_{0}$. After the first elimination the honest coarse object is generally not another simplicial complex: it is an operator-valued response network $\mathcal{R}_{\ell+1}$ whose vertices carry vector spaces and whose links carry linear response blocks. A cellular sheaf on the quotient graph is a natural realization when the blocks admit compatible restriction-map factorization \[10\]; otherwise Tessera retains the more general response network and does not invent incidence maps that the reduction did not determine. The data type stable under the recursion is a complex symmetric pencil with an inherited chain metric: with the retained fibers $J$ and their geometric images $Z=G_1J$, the coarse metric is $\mathcal G_{\ell+1}=J^{\mathsf T}G_1J=Z^{\mathsf T}M_1Z$ and the coarse pencil $J^{\mathsf T}A_1J=Z^{\mathsf T}\tilde A_1Z$, the chain metric and pencil restricted to the retained fibers, and an off-diagonal block $Z_A^{\mathsf T}M_1Z_B$ vanishes unless the supports of the two geometric images share a top simplex, so overlaps are local when cluster supports are defined on the images (specification \[43\], Prop. 7.1).
...
- a closed complex-plane contour with nonzero separation and a controlled resolvent separating it from discarded modes, taken at the reported rotation $\varepsilon>0$ for a Lorentzian complex (specification \[43\], §10);
...
#### Adopted scope decision: a two-body cobordism map (proposed identification).

Item 4 is adopted as follows, with its certificates. Two prepared input fibers are attached to two vertex-disjoint cell sets $A$ and $B$ of an interaction cobordism $W$ grown from a single simplex (the attachment order is the attachment permutation), and the bulk of $W$ between them is read as the transfer between the full frames on $A$ and $B$, $T_{AB}=(Z_A^{\vee})^{\mathsf T}(\tilde A^{U})_{AB}Z_B$ with unit images: the coupling block of the whole between the two frames. Two readings are kept side by side and selected explicitly: the Choi-decomposed reading treats $\mathrm{vec}(T_{AB})$ as a state on the pair space of the two frames, the operator reading treats $T_{AB}$ as a map between them; the fit of a prescribed two-body output $\chi$ is the projective Frobenius leak in either. The certificates are the Schmidt spectrum of $T_{AB}$ (rank one for a product, the signature of a quasi-free image; rank two for the flip-flop below), the reversal identity $T_{BA}(U^{-1})=T_{AB}(U)^{\mathsf T}$, and the fiber residuals of the two inputs. The test system is two spin-$3/2$ systems under the XY exchange coupling $H_{\mathrm{int}}=\hbar J(\Sigma_1^{+}\Sigma_2^{-}
+\Sigma_1^{-}\Sigma_2^{+})$, whose dimensionless bilinear map has the image $\chi=(D\psi)(U\varphi)^{\mathsf T}+(U\psi)(D\varphi)^{\mathsf T}$ on a product input, with $D$ the matrix of $\Sigma^{-}$ and $U=D^{\mathsf T}$; its first-order amplitude to an orthogonal final state is $-iJt\,\chi_{KM}$, its branching ratios are exact at leading order, its selection rule $K+M=k+m$ persists to all orders, and its exact amplitudes are the $N$-block exponentials. $\chi$ changes both particles at once and is quartic in level modes: it is not the second quantization of a one-particle map, which is why its realization by an emergent bulk is a scope decision and not a consequence of the quasi-free closure. One cobordism is the first-order velocity; whether a bulk synthesized on one input pair reproduces $\chi$ of other inputs attached to it is the measurement, not a theorem.
...
A hopping-defined transfer reverses by transposition: on the pencil, $T_{BA}(U^{-1})=T_{AB}(U)^{\mathsf T}$, and $T_{BA}=T_{AB}^{\mathsf T}$ at $U=1$ (specification \[43\], Prop. 7.1). The branch-free dual transport $$M_{AB}^{\vee}=M_{AB}^{-\mathsf T},\qquad
  \det M_{AB}^{\vee}=(\det M_{AB})^{-1},$$ follows only under the additional groupoid hypothesis $T_{BA}=T_{AB}^{-1}$; otherwise the sign flip of the determinant winding comes from the closure convention. This is the transport assigned below to an anti-cluster. Pair creation conserves total determinant winding under any continuous dual-pair homotopy that avoids $\det M=0$ and boundary flux.
...
For a second fully invariant coordinate, the connection-dressed face endomorphism is realized on chains by the Whitney block of $\tau$, $$\Pi_\tau(U)=G_1^{U}M_1^{(\tau)U}G_1^{U},\qquad
  (M_1^{(\tau)U})_{ee'}=(M_1^{(\tau)})_{ee'}\,U_{b(e)b(e')},$$ where $M_1^{(\tau)}\in\mathbb C^{3\times3}$, the contribution of $\tau$ to $M_1$ restricted to its three edges, is nonsingular for a nondegenerate triangle: the three Whitney 1-forms of $\tau$ are linearly independent functions on $\tau$ and $M_1^{(\tau)}$ is their Gram matrix in a nondegenerate metric, so the block has rank three (specification \[43\], Prop. 8.1). It satisfies $\Pi_\tau(U^g)=\rho_1(g)\Pi_\tau(U)\rho_1(g)^{-1}$. Define $$\alpha_\tau=
  \det\!\left(
    \widetilde\Phi_Q^{\mathsf T}
    \Pi_\tau(U)\Phi_Q
  \right)\in\mathbb C .$$ The inner matrix transforms by similarity under both gauge groups, so $\alpha_\tau$ is invariant; paired through the geometric images it is $\alpha_\tau=\det\bigl((Z_Q^{\vee})^{\mathsf T}M_1^{(\tau)U}Z_Q\bigr)$, a well-defined complex number that is generically nonzero for a rank-three fiber (specification Prop. 8.1). The anchor certificate is the persistent nonzero projective profile $[\Delta_Q]$, the complex profile $\{\alpha_\tau\}$, and determinant-line transition functions on overlaps. An extended fiber may be anchored by an atlas of faces; concentration on one face is not required. Numerical projective distances and condition numbers may be reported to certify stability, but no modulus, square root, free face weight, or real-valued score enters the physical definition. If the required connection-dressed covariance cannot be verified, the anchor refuses rather than reporting a gauge-dependent raw restriction. Because every 2-simplex has exactly three boundary edges, the construction is independent of ambient spectral dimension.

#### The tetrahedral anchor at degree zero (proposed identification).

The same construction one degree down and one dimension up anchors a four-level system to a 3-simplex $T$. The per-tetrahedron Whitney block at degree zero, $M_0^{(T)}\in\mathbb C^{4\times4}$, is the Gram matrix of the four barycentric functions of $T$, $\mathrm{vol}(T)\,(I+\mathbf 1\mathbf 1^{\mathsf T})/20$ at $d=3$ and the sum of the local blocks of the top simplices containing $T$ restricted to its vertices at $d\ge4$; it has rank four for a nondegenerate tetrahedron. With the connection, $$\Pi_T(U)=G_0^{U}M_0^{(T)U}G_0^{U},\qquad
  (M_0^{(T)U})_{vw}=(M_0^{(T)})_{vw}\,U_{vw},$$ since $b(v)=v$ for a vertex, and the anchor coordinate of a rank-four fiber is $\alpha_T=\det\bigl((Z_Q^{\vee})^{\mathsf T}M_0^{(T)U}Z_Q\bigr)$. No dagger enters: by the transpose identity $(M_k^{U})^{\mathsf T}=M_k^{U^{-1}}$ (specification \[43\], Prop. 5.1(ii)) the left factor of $\Pi_T$ is already the transposed dual-side metric, so the form is covariant under the transpose pairing as written, and a conjugate-transpose would carry $\bar U$ and break it. The Grassmann degree-zero block pairs scalars and has rank one, so the tetrahedral anchor is defined for the Whitney metric only and refuses that preset by name. The degree-zero harmonic band at trivial holonomy is the vertex-volume chain $M_0\mathbf 1$, whose geometric image is the constant vector and which therefore carries no spectral content; a certificate reports how much of it a selected band contains (the Riesz projection of the constant image under the pencil pairing: one on that band, exactly zero on a band of other eigenvalues). Nothing is excluded by it: with a connection carrying flux the zero mode is lifted and the lowest band is a genuine ground band. This is a proposed identification, not a derivation: it succeeds only if the anchored degree-zero band of a grown 3-complex carries a prescribed four-level state by its lengths and connection values alone.
...
#### Time as the recursion (proposed identification).

With spatial simplices at every level, one interaction cobordism represents the first-order change of the carried state (the velocity of Section <a href="#sec:quasifree" data-reference-type="ref" data-reference="sec:quasifree">7</a>), and time evolution is proposed to be the succession of levels, each consuming the previous level’s output fiber through the same continuation that the recursion below defines. Nothing beyond first order is obtained from one level; the exact amplitudes of the test system are its $N$-block exponentials, and agreement of an $n$-level succession with them is the measurement that would support the identification. It is stated as a proposal, not a derivation.
...
The geometric subspaces $E_{v}\subset C(K)$ of adjacent components may overlap on shared interface cells, so their internal sum need not be direct. The recursion therefore never asserts $\bigoplus_{v}E_{v}\subset C(K)$. It forms the abstract labeled sum, carries the embedding $J_{\ell+1}$, its algebraic dual, and overlap matrix $G_{\ell+1}$ exactly, and proceeds by exactly one of three declared options: carry $G$ in every subsequent formula; certify $\lVert G-I\rVert\le\varepsilon$ and propagate $\varepsilon$ through the composable amplitude budget of Section <a href="#sec:interactions" data-reference-type="ref" data-reference="sec:interactions">6</a>; or quotient $G$’s left and right radicals through a rank-revealing factorization and restate the fiber ranks. On the Whitney pencil the three options collapse to the first at no cost: $G_{\ell+1}=J^{\mathsf T}G_1J$ is the $L^{2}$ inner product restricted to the retained fibers, carried exactly, and it is local when fiber supports are defined on geometric images, an off-diagonal block vanishing unless two image supports share a top simplex (specification \[43\], Prop. 7.1). A sheaf-stalk decomposition that assigns interface modes to link stalks is a valid realization of the same requirement \[10\], but it is not necessary.
...
\[43\] Recursive Spectral Fibers from the Chain-Level Whitney Hodge Pencil: integration specification (chain formulation). Tessera cobordism programme, `docs/design/rsf_whitney_integration_spec.tex`.

\[44\] Scaling Verification Plan for the Chain-Level Hodge Construction. Tessera cobordism programme, `docs/design/scaling_verification_plan.tex`.
