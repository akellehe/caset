...
and [charged_cartan_monte_carlo_v0.1.md](charged_cartan_monte_carlo_v0.1.md).
...
- **Practical, concrete**: a textbook QFT we could write down without
  ...
  established meaning; a well-documented `g → 0` (strong-coupling)
  limit where confinement and meson-like bound states appear.
- **Structural**: the Hamiltonian commutes with the staggered fermion
  number `Q` per site-pair, so charge is a real conserved U(1) quantum
  number rather than a bookkeeping label.
...
1. **Providing a 4×4 unitary `U`** to apply to each pair of inputs.
...
step works the same way for *any* 4×4 unitary — the Schwinger choice
just fixed a specific point in the SU(4) parameter space.

## What we already truncated by going two-site

The Schwinger model in 1+1D has a **non-local Coulomb term** `L_n²`
(the gauge-field electric energy) that, after Gauss's law is solved,
becomes a long-range `σ^z_i σ^z_j` interaction between fermion sites
weighted by `|i − j|`. This term:
...
`exp(i (c_x XX + c_y YY + c_z ZZ))`. The Cartan decomposition exists
for any 4×4 unitary, so this is just choosing a representative point
in SU(4) without the appearance of physical baggage. We keep the
Schwinger U as an alternative option (`cfg.unitaryMode = SCHWINGER`)
...
- v0.2 promotes charge to be intrinsic to the state by using a 4-dim
  qudit basis `{|+0⟩, |+1⟩, |−0⟩, |−1⟩}` with `Q̂ = diag(+1, +1, −1, −1)`,
  recovering the operator-level statement.
...
- **Two-site interaction unitaries on density-matrix inputs.** Every
  interaction is `ρ_AB = U (ρ_A ⊗ ρ_B) U†` with `U` a fixed 4×4
  unitary. The choice of `U` parameterises the SU(4) Cartan core but
  doesn't change the structural content of the experiment.
- **The Cartan / KAK decomposition** `U = (K₁ ⊗ K₂)·exp(i·c·σσ)·(K₃ ⊗ K₄)`
  as the principled way to split per-cell physics into "local-frame
  carriers" (the U_A, V_B worldline products) and "entangling core"
  (the Σ_AB joint-state carrier).
- **Mutual-information-derived edge lengths** `ℓ = -log(I / I_max)` as
  the way to lift quantum correlation onto geometric edges of a
  simplicial complex.
- **Regge action on hinges of the resulting complex** as the dynamical
  weight, with Monte Carlo sampling at inverse temperature `β`.
...
## Why this is the right framing for the H_DS4 question

The hypothesis `H_DS4` — "is there a `β` at which the heat-kernel
spectral dimension of the interaction-history complex reaches 4?" —
is a *geometric* question. It depends on:
...
by an Euclidean Regge action, and sampled at inverse temperature β.
Charges are a separately-tracked U(1) quantum number, conserved by
the move rules.
