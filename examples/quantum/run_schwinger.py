...
Convergence diagnostics: the DMRG result's bond_dim field reports the
...
def _config_for(N: int, m_over_g: float, x: float,
                L0: float, max_bond_dim: int, n_sweeps: int) -> QuantumConfig:
...
def _print_row(N: int, m_over_g: float, x: float, L0: float,
    ...
    print(f"{N:>4} {m_over_g:>6.3f} {x:>6.2f} {L0:>5.2f} "
          ...
          f"{result.bond_dim:>9d} {result.truncation_err:>11.2e}")


def _run_one(N: int, m_over_g: float, x: float, L0: float,
             max_bond_dim: int, n_sweeps: int) -> None:
    cfg = _config_for(N, m_over_g, x, L0, max_bond_dim, n_sweeps)
...
def _scan_x(N: int, m_over_g: float, xs: Iterable[float], L0: float,
    ...
    _print_header()
    for x in xs:
        _run_one(N, m_over_g, x, L0, max_bond_dim, n_sweeps)


def _scan_bond_dim(N: int, m_over_g: float, x: float, L0: float,
                   bond_dims: Iterable[int], n_sweeps: int) -> None:
    print(f"\nVariational descent: energy non-increasing in max_bond_dim")
    _print_header()
    for D in bond_dims:
        _run_one(N, m_over_g, x, L0, D, n_sweeps)
...
def main() -> None:
    ...
    p.add_argument("--scan-bond-dim", type=int,  nargs="+", default=None,
                   metavar="D",
                   help="Scan over max_bond_dim values (overrides --max-bond-dim).")
    args = p.parse_args()
